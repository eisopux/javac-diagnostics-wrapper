...
/**
 * Range in a text document expressed as (zero-based) start and end positions. A range is comparable
 * to a selection in an editor. Therefore the end position is exclusive. If you want to specify a
 * range that contains a line including the line ending character(s) then use an end position
...
public class Range {
    ...
    /**
     * Define a position in a file using the x and y coordinates y coordinates = line number x
     * coordinates = offset from the beginning of the line
     *
     * <p>The positions in a text document use zero-based line and zero-based character offset. A
     * position is between two characters like an "insert" cursor in a editor. Special values like
    ...
    private static class Position {
         ...
         * represented as a string, the `character` value represents the gap between the `character`
         * and `character + 1`.
         */
